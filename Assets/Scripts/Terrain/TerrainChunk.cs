...
namespace Minecraft
{
    ...
    public sealed class TerrainChunk : MonoBehaviour
    {
        ...
        {
            return IsInThisChunkPhysically(blockLocalIndex)
                ? blocks[blockLocalIndex.X, blockLocalIndex.Y, blockLocalIndex.Z] : null;
        }
        ...
        public void SetBlockType(Index3D blockLocalIndex, TerrainBlock.Type newBlockType)
        {
            ...
            if (IsInThisChunkPhysically(blockLocalIndex))
            {
        ...
        public bool IsInThisChunkLogically(Index3D blockLocalIndex)
        {
        ...
        private bool IsInThisChunkPhysically(Index3D blockLocalIndex)
        {
            return blockLocalIndex.X >= 0 && blockLocalIndex.X < blocks.GetLength(0)
                && blockLocalIndex.Y >= 0 && blockLocalIndex.Y < blocks.GetLength(1)
                && blockLocalIndex.Z >= 0 && blockLocalIndex.Z < blocks.GetLength(2);
        }
